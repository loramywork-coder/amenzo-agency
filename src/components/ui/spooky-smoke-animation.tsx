...
void main(){
  ...
  n=noise(uv*3.+n*2.);
  col.r-=fbm(uv*0.97+vec2(0,T*.015)+n-.02);
  col.g-=fbm(uv*1.0+vec2(0,T*.015)+n);
  col.b-=fbm(uv*1.04+vec2(0,T*.015)+n+.035);
  col=mix(col, u_color*1.3, dot(col,vec3(.21,.71,.07)));
  col=mix(vec3(.01),col,min(time*.18,1.));
  col=clamp(col,.01,1.);
  O=vec4(col,1);
}`;
...
void main(){gl_Position=position;}`;
  ...
  private color: [number, number, number] = [0.3, 0.3, 0.3];
...
export const SmokeBackground: React.FC<SmokeBackgroundProps> = ({ smokeColor = "#404040" }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const rendererRef = useRef<Renderer | null>(null);
  const pathname = usePathname();
  const isHidden = pathname.startsWith("/demos") || pathname.startsWith("/showcases");

  useEffect(() => {
    if (isHidden || !canvasRef.current) return;
  ...
  return <canvas ref={canvasRef} className="fixed inset-0 w-full h-full z-0 pointer-events-none" style={{ opacity: 1 }} />;
};
