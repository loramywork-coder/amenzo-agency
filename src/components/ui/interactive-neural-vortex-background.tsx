  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        color = mix(color, vec3(0.7, 0.55, 0.2), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.15, 0.0, 0.4) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
    ...
    const vertexShader = compileShader(glCtx, vsSource, glCtx.VERTEX_SHADER);
    const fragmentShader = compileShader(glCtx, fsSource, glCtx.FRAGMENT_SHADER);
    ...
    const render = () => {
      ...
      glCtx.uniform1f(uTime, slowTime);
      glCtx.uniform2f(
        uPointerPosition,
        pointer.current.x / window.innerWidth,
        1 - pointer.current.y / window.innerHeight
      );
      glCtx.uniform1f(uScrollProgress, window.pageYOffset / (2 * window.innerHeight));
